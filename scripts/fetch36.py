...
for i, title in enumerate(titles):
    for kw in KEYWORDS:
        if kw in title:
            ...
            results.append({'title': title, 'link': link, 'pubdate': pubdate, 'keyword': kw})
            break

print(f'[{datetime.now().strftime("%H:%M")}] 36kr RSS: {len(results)} items matching keywords')
for r in results[:5]:
    print(r['title'][:50], '|', r['link'][:50], '|', r['pubdate'])
