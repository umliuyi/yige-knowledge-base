...
# Decode as GBK for body text
raw_gbk = raw_bytes.decode('gbk', errors='ignore')

# Extract og:title from GBK-decoded string (it has correct Chinese here!)
og_t = re.search(r'<meta property="og:title" content="([^"]+)"', raw_gbk)
og_d = re.search(r'<meta property="og:description" content="([^"]+)"', raw_gbk)
og_i = re.search(r'<meta property="og:image" content="([^"]+)"', raw_gbk)
