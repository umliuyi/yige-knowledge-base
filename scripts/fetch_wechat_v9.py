...
# Extract og:title from GBK-decoded page (it's correct in GBK)
gbk_og = re.search(r'<meta property="og:title" content="([^"]+)"', raw_gbk)
gbk_og_title = gbk_og.group(1) if gbk_og else ''
# Convert GBK title string back to bytes, then decode as UTF-8
title_bytes = gbk_og_title.encode('gbk', errors='replace')
...
# Extract paragraphs from body (GBK has more Chinese)
...
for t in texts:
    ...
    if len(txt) >= 10 and re.search(r'[\u4e00-\u9fff]', txt):
        ...
        # Also try to fix remaining garbled chars by round-tripping through GBK bytes
        fixed = txt.encode('gbk', errors='replace').decode('utf-8', errors='replace')
        clean.append(fixed)
