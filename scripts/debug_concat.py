import subprocess, os, sys

ffmpeg = r"C:\Program Files\AutoClaw\resources\python\Lib\site-packages\imageio_ffmpeg\binaries\ffmpeg-win-x86_64-v7.1.exe"
base = r"C:\Users\Administrator\Downloads\videos\daily_news\2026-05-11-v3"

# Check segments
segs = [os.path.join(base, f"seg_{i}.mp4") for i in range(4)]
seg_files = [s for s in segs if os.path.exists(s)]
print("Segments:", [f"{os.path.getsize(s)//1024}KB" for s in seg_files])

# Write concat list
list_file = os.path.join(base, "segments2.txt")
with open(list_file, "w") as f:
    for s in seg_files:
        f.write(f"file '{s}'\n")

print("List content:", open(list_file).read())

out = os.path.join(base, "daily_news_20260511_v6.mp4")
cmd = [
    ffmpeg, "-y",
    "-f", "concat", "-safe", "0", "-i", list_file,
    "-c:v", "libx264", "-crf", "20", "-pix_fmt", "yuv420p",
    "-c:a", "aac", "-b:a", "128k",
    out
]
print("Cmd:", cmd[:5])
r = subprocess.run(cmd, capture_output=True, text=True)
print("RC:", r.returncode)
if r.returncode != 0:
    print("Stderr:", r.stderr[-500:] if r.stderr else "")
if os.path.exists(out):
    sz = os.path.getsize(out)
    print(f"Output: {sz//1024}KB ({sz//1024//1024}MB)")
else:
    print("Output not created")
