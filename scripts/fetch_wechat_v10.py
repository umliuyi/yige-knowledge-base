import urllib.request, ssl, re, json, html as html_module, sys
from datetime import datetime

# Fix stdout for GBK console
...
# Meta tags (og:title/og:desc/og:image) in UTF-8
...
title_utf8 = og_t.group(1) if og_t else ''
desc_utf8 = og_d.group(1) if og_d else ''
img = og_i.group(1) if og_i else ''

# Body paragraphs in GBK
...
print('Title:', title_utf8[:60])
print('Desc:', desc_utf8[:60])
...
with open(out_raw, 'w', encoding='utf-8') as f:
    f.write('Title: ' + title_utf8 + '\nURL: ' + url + '\nDate: ' + datetime.now().strftime('%Y-%m-%d') + '\n\n' + full)
with open(out_json, 'w', encoding='utf-8') as f:
    json.dump({'title': title_utf8, 'desc': desc_utf8, 'url': url, 'img': img, 'date': datetime.now().strftime('%Y-%m-%d'), 'paragraphs': clean}, f, ensure_ascii=False, indent=2)
print('Saved')